<template>
  ...
  <div class="min-h-screen bg-[#0d1117] text-[#c9d1d9] font-sans antialiased">
    <!-- Top Navigation Bar -->
    <header class="fixed top-0 w-full z-50 bg-[#161b22] border-b border-[#30363d] h-14 shadow-sm">
      <div class="max-w-[1264px] mx-auto px-4 h-full flex items-center justify-between gap-4">
        <!-- Logo -->
        <Link href="/"
          class="flex items-center gap-1 hover:bg-[#21262d] px-2 py-1 rounded transition-colors duration-200 group">
        <svg class="w-7 h-7 text-orange-500 group-hover:text-orange-400 transition-colors" fill="currentColor"
          viewBox="0 0 24 24">
        ...
        <div class="flex-1 max-w-2xl relative group">
          <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            <svg class="h-4 w-4 text-[#8b949e] group-focus-within:text-[#58a6ff]" fill="none" viewBox="0 0 24 24"
              stroke="currentColor">
        ...
        <div class="flex items-center gap-2">
          <template v-if="$page.props.auth?.user">
            ...
            <!-- Avatar Placeholder -->
            <div class="w-5 h-5 bg-green-500 rounded flex items-center justify-center text-[10px] text-white font-bold">
              {{ $page.props.auth.user.name.charAt(0) }}
            </div>
            <span>{{ $page.props.auth.user.name }} </span>
            </Link>
            <form @submit.prevent="logout">
              <button type="submit" class="text-white bg-red-500 text-sm px-2 py-1 rounded-xl">
                logout
              </button>
            </form>
          </template>
    ...
    <!-- Layout Container -->
    <div class="max-w-[1264px] mx-auto w-full pt-14 flex justify-center text-left">
      <!-- Left Sidebar (Navigation) -->
      <nav class="hidden md:block w-[164px] flex-shrink-0 pt-8 pr-2 sticky top-14 h-[calc(100vh-3.5rem)] text-[13px]">
        <ul class="space-y-1">
          <li>
            <Link href="/"
          ...
          <li class="mt-4 px-2 text-[11px] font-bold text-[#8b949e] uppercase tracking-wider mb-1">
            Public
          </li>
          <li>
            <Link href="#"
      ...
      <main class="flex-1 min-w-0 max-w-[750px] w-full border-l border-[#30363d] md:border-l-0">
        <slot />
      </main>
      ...
      <aside class="hidden xl:block w-[300px] flex-shrink-0 ml-6 pt-6">
        <!-- Blog Widget -->
        <div class="bg-[#161b22] border border-[#f1e05a] rounded-[3px] mb-4 text-[13px] overflow-hidden shadow-sm">
          <div
          ...
          <ul class="list-none p-0 bg-[#0d1117]/30">
            <li v-for="question in $page.props.latest_questions" :key="question.id"
              class="flex items-start gap-2 px-3 py-2.5 hover:bg-[#21262d] cursor-pointer transition-colors">
              <svg class="w-4 h-4 text-[#8b949e] flex-shrink-0 mt-0.5" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Featured Widget -->
        <div class="bg-[#161b22] border border-[#30363d] rounded-[3px] mb-4 text-[13px] shadow-sm">
          <div
            class="bg-[#21262d] border-b border-[#30363d] px-4 py-2 font-bold text-[#8b949e] text-[12px] uppercase tracking-wide">
            Featured on Meta
          </div>
          <ul class="list-none p-0">
            <li class="flex items-start gap-2 px-3 py-2.5 hover:bg-[#21262d] cursor-pointer transition-colors">
              <svg class="w-4 h-4 text-[#58a6ff] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path d="M2 5a2 2 0 012-2h7a2 2 0 012 2v4a2 2 0 01-2 2H9l-3 3v-3H4a2 2 0 01-2-2V5z"></path>
                <path
                  d="M15 7v2a4 4 0 01-4 4H9.828l-1.766 1.767c.28.149.599.233.938.233h2l3 3v-3h2a2 2 0 002-2V9a2 2 0 00-2-2h-1z">
                </path>
              </svg>
              <span class="text-[#c9d1d9] leading-snug">New responsive search UI</span>
            </li>
            <li class="flex items-start gap-2 px-3 py-2.5 hover:bg-[#21262d] cursor-pointer transition-colors">
              <svg class="w-4 h-4 text-[#58a6ff] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                <path d="M2 5a2 2 0 012-2h7a2 2 0 012 2v4a2 2 0 01-2 2H9l-3 3v-3H4a2 2 0 01-2-2V5z"></path>
              ...
              <span class="text-[#c9d1d9] leading-snug">Colors update in Dark Mode 2.0</span>
            </li>
          </ul>
        </div>

        <!-- Custom Tags -->
        <div class="bg-[#161b22] border border-[#30363d] rounded-[3px] p-4 shadow-sm">
          <h4 class="text-[#c9d1d9] font-bold text-[15px] mb-3">
            Popular Tags
          </h4>
          <div class="flex flex-wrap gap-2">
            <Link v-for="tag in $page.props.tags" :key="tag.id" :href="'/?tag=' + tag.id">
            <span
