<template>
    <div
        class=" my-4 min-h-screen bg-[#0d1117] text-gray-300 font-sans selection:bg-indigo-500 selection:text-white pb-20">
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-10">
            ...
            <div class="">
                ...
                <div class="flex-1 pt-20 md:pt-24 text-center md:text-left">
                    ...
                    <!-- Meta Info Row -->
                    <div
                        class="flex flex-wrap items-center justify-center md:justify-start gap-x-6 gap-y-3 text-sm text-gray-500 mb-8">
            ...
            <div class="mt-10">
                ...
                <div class="lg:col-span-4 space-y-6">
                    ...
                    <div class="bg-[#161b22] border border-gray-800 rounded-xl p-6 shadow-sm">
                        <h3 class="text-sm font-semibold text-gray-100 uppercase tracking-wider my-5">Community Stats
                        </h3>
                        <div class="flex justify-between gap-4 mt-5">
                            <div
                                class="p-4 w-full bg-[#0d1117] rounded-lg border border-gray-800/50 hover:border-gray-700 transition-colors">
                ...
                <div class="lg:col-span-8">
                    ...
                    <div class="flex items-center border-b border-gray-800 mb-6">
                        <button @click="activeTab = 'all'"
                            ...
                            :class="activeTab === 'all' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-200'">All
                            Activity</button>
                        <button @click="activeTab = 'questions'"
                            class="px-6 py-3 text-sm font-medium border-b-2 transition-colors duration-200"
                            :class="activeTab === 'questions' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-200'">Questions</button>
                        <button @click="activeTab = 'answers'"
                            class="px-6 py-3 text-sm font-medium border-b-2 transition-colors duration-200"
                            :class="activeTab === 'answers' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-200'">Answers</button>

                        <button @click="activeTab = 'recentQuestions'"
                            class="px-6 py-3 text-sm font-medium border-b-2 transition-colors duration-200"
                            :class="activeTab === 'recentQuestions' ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-gray-400 hover:text-gray-200'">Recent
                            Questions</button>
                    ...
                    <div class="space-y-4">
                        <div v-if="filteredPosts.length === 0"
                            class="text-center py-12 bg-[#161b22] rounded-xl border border-gray-800 border-dashed">
                            <div
                                class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gray-800 mb-3">
                        ...
                        <div v-for="post in filteredPosts" :key="post.uniqueId"
                            ...
                            <div class="flex-shrink-0 mt-1">
                                <span v-if="post.type === 'question'"
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-emerald-500/10 text-emerald-500 ring-1 ring-emerald-500/20">
                                    <span class="font-bold text-xs">Q</span>
                                </span>
                                <span v-else
                                    class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-indigo-500/10 text-indigo-500 ring-1 ring-indigo-500/20">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center justify-between gap-4 mb-1">
                                    <Link
                                        :href="`/questions/${post.routeId}/${post.type === 'answer' ? '#answerId-' + post.answerId : ''}`"
                                        class="text-base font-semibold text-gray-200 group-hover:text-indigo-400 transition-colors line-clamp-1">
                                    {{ post.title }}
                                    </Link>
                                    <span class="text-xs text-gray-500 whitespace-nowrap">{{ formatDate(post.date)
                                        }}</span>
                                </div>
                    ...
                    <div class="mt-6 flex justify-center" v-if="filteredPosts.length > 0">
                        <button class="text-sm text-indigo-400 hover:text-indigo-300 font-medium transition-colors">Load
...
export default {
    ...
    computed: {
        filteredPosts() {
            ...
            const rquestions = this.recentQuestions.map(rq => ({
                uniqueId: 'rq-' + rq.id,
                routeId: rq.id,
                type: 'question',
                title: rq.title,
                body: rq.body,
                date: rq.pivot.viewed_at,
                votes: 0 // Placeholder
            }));
            ...
            if (this.activeTab === 'all') {
                all = [...questions, ...answers, ...rquestions];
            } else if (this.activeTab === 'questions') {
                all = questions;
            } else if (this.activeTab === 'answers') {
                all = answers;
            } else if (this.activeTab === 'recentQuestions') {
                all = rquestions;
            }
